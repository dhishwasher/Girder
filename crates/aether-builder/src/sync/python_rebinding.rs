...
//! name is rebound -- by a string literal or by a dotted attribute
//! assignment/delete -- living in a DIFFERENT file than the target's own
//! definition.
//!
//! `claims.rs`'s own per-file `annotate()` pass already excludes a
//! same-file rebinding (`setattr(obj, "target", ...)`,
//! `patch("pkg.mod.target")`, `mod.target = ...`, `del mod.target`) from
//! being proven Must -- but it has no visibility into any OTHER file.
//! Checked directly against the real audited packages, not just
//! theoretically: a cross-file `mocker.patch('pkg.module.target')` in a
//! test file, naming a function defined (and called from, elsewhere in the
//! SAME file) in `pkg/module.py`, would otherwise leave that same-file call
//! site incorrectly proven Must. This pass runs after all files are
//! extracted, builds a crate-wide index of every file's string literals and
//! attribute-rebind targets, and reverts any `proven-top-level-lexical-
//! binding` Must claim whose target's own name is rebound ANYWHERE in the
//! indexed project -- not just the target's own file.
...
/// any string literal in `all_string_literals`, or is a bare attribute-
/// rebind target in `all_attribute_rebind_targets`.
fn rebound(
    name: &str,
    all_string_literals: &HashSet<String>,
    all_attribute_rebind_targets: &HashSet<String>,
) -> bool {
    all_string_literals
        .iter()
        .any(|s| s == name || s.ends_with(&format!(".{name}")))
        || all_attribute_rebind_targets.contains(name)
}
...
pub(super) fn revert_string_rebound_python_claims(
    ...
    let mut all_string_literals = HashSet::new();
    let mut all_attribute_rebind_targets = HashSet::new();
    for state in files.values() {
        all_string_literals.extend(state.extraction.python_string_literals.iter().cloned());
        all_attribute_rebind_targets.extend(
            state
                .extraction
    ...
    if all_string_literals.is_empty() && all_attribute_rebind_targets.is_empty() {
        return; // No Python files, or none contain any rebinding hazard at all.
    }
...
mod tests {
    ...
    #[test]
    fn a_same_file_must_claim_is_reverted_when_a_different_file_rebinds_the_target_by_attribute_assignment(
    ) {
        let files = [
            (
                "pkg/module.py",
            ...
            (
                "tests/test_module.py",
                "def test_it():\n\
                 \x20\x20\x20\x20pkg.module.target = lambda: 0\n",
            ),
        ];
        let mut graph = SemanticGraph::new();
        let mut builder = GraphBuilder::new();
