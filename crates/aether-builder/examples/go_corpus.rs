...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let root = PathBuf::from(args.next().ok_or("usage: go_corpus ROOT OUTPUT")?);
    let output = PathBuf::from(args.next().ok_or("usage: go_corpus ROOT OUTPUT")?);
    if args.next().is_some() {
        return Err("usage: go_corpus ROOT OUTPUT".into());
    }
    ...
    builder.set_go_module_path(Some(module_path));
    builder.load_files(
        &mut graph,
        sources
            .iter()
...
fn read_module_path(root: &Path) -> std::io::Result<String> {
    ...
    source
        ...
        .ok_or_else(|| std::io::Error::other("go.mod does not declare a module path"))
}
...
fn collect_paths(root: &Path, directory: &Path, out: &mut Vec<PathBuf>) -> std::io::Result<()> {
    ...
    for entry in entries {
        ...
        if file_type.is_dir() {
            ...
            && Lang::from_path(&relative.to_string_lossy()) == Some(Lang::Go)
        {
            out.push(relative);
        }
    }
