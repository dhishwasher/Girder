//! EXTENSION POINT — Gemini provider.
//!
...
//! `--features live-providers` and setting `GEMINI_API_KEY` is where you'd wire the real
...
impl GeminiProvider {
    /// Reads the key from `GEMINI_API_KEY` if present.
...
impl AiProvider for GeminiProvider {
    ...
    async fn complete(&self, _prompt: Prompt) -> Result<Completion, AiError> {
        #[cfg(feature = "live-providers")]
        {
            ...
            let _ = &self.api_key;
            let _ = &self.model;
            return Err(AiError::Unsupported("google:gemini".into()));
        }
