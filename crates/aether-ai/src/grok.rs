...
//! `--features live-providers` and setting `XAI_API_KEY` is where you'd wire the real
...
impl GrokProvider {
    /// Reads the key from `XAI_API_KEY` if present.
    pub fn from_env() -> Self {
        GrokProvider {
            api_key: std::env::var("XAI_API_KEY").ok(),
            model: "xai:grok".to_string(),
        }
    }
...
impl AiProvider for GrokProvider {
    ...
    async fn complete(&self, _prompt: Prompt) -> Result<Completion, AiError> {
        #[cfg(feature = "live-providers")]
        {
            ...
            let _ = &self.api_key;
            let _ = &self.model;
            return Err(AiError::Unsupported("xai:grok".into()));
        }
        #[cfg(not(feature = "live-providers"))]
        Err(AiError::Unsupported("xai:grok".into()))
    }
}
