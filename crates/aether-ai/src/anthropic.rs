//! EXTENSION POINT — Anthropic provider.
//!
...
//! `--features live-providers` and setting `ANTHROPIC_API_KEY` is where you'd wire the real
...
impl AiProvider for AnthropicProvider {
    ...
    async fn complete(&self, _prompt: Prompt) -> Result<Completion, AiError> {
        #[cfg(feature = "live-providers")]
        {
            ...
            let _ = &self.api_key;
            let _ = &self.model;
            return Err(AiError::Unsupported("anthropic:claude".into()));
        }
        #[cfg(not(feature = "live-providers"))]
        Err(AiError::Unsupported("anthropic:claude".into()))
    }
}
