//! EXTENSION POINT — Ollama provider.
//!
//! A real, compile-clean struct that slots into the [`AiProvider`] trait. The
//! HTTP body is intentionally not implemented in the prototype: building with
//! `--features live-providers` and setting `OLLAMA_HOST` is where you'd wire the real
//! API call (reqwest + the provider's JSON schema). Until then it reports
//! [`AiError::Unsupported`] so the swarm transparently falls back to the mock.

use crate::provider::{AiError, AiProvider, Completion, Prompt, TaskClass};
use async_trait::async_trait;

pub struct OllamaProvider {
    api_key: Option<String>,
    model: String,
}

impl OllamaProvider {
    /// Reads the key from `OLLAMA_HOST` if present.
    pub fn from_env() -> Self {
        OllamaProvider {
            api_key: std::env::var("OLLAMA_HOST").ok(),
            model: "ollama:local".to_string(),
        }
    }

    pub fn with_model(mut self, model: impl Into<String>) -> Self {
        self.model = model.into();
        self
    }
}

#[async_trait]
impl AiProvider for OllamaProvider {
    fn name(&self) -> &str {
        "ollama:local"
    }

    fn handles(&self, _class: TaskClass) -> bool {
        // Extension point only: do not enter routing until complete() performs
        // a real Ollama request.
        let _configured = self.api_key.is_some();
        false
    }

    async fn complete(&self, _prompt: Prompt) -> Result<Completion, AiError> {
        #[cfg(feature = "live-providers")]
        {
            // EXTENSION POINT: real request goes here, e.g.
            //   let client = reqwest::Client::new();
            //   let resp = client.post(ENDPOINT).bearer_auth(key).json(&body).send().await?;
            // For now even the live build is unimplemented.
            let _ = &self.api_key;
            let _ = &self.model;
            return Err(AiError::Unsupported("ollama:local".into()));
        }
        #[cfg(not(feature = "live-providers"))]
        Err(AiError::Unsupported("ollama:local".into()))
    }
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn extension_point_declines_routing_even_when_configured() {
        let provider = OllamaProvider {
            api_key: Some("http://localhost:11434".to_string()),
            model: "test-model".to_string(),
        };
        assert!(!provider.handles(TaskClass::Quick));
    }
}
