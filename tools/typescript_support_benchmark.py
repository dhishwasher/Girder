#!/usr/bin/env python3
"""Run the frozen TypeScript support policy against its pinned corpora."""
...
DEFAULT_POLICY = REPO_ROOT / "docs" / "typescript-support-policy.json"
DEFAULT_CACHE = REPO_ROOT / ".benchmark-cache" / "typescript-support-v1"
DEFAULT_OUTPUT = REPO_ROOT / "docs" / "typescript-support-observation.json"
...
def evaluate_cases(
    ...
    """Classify probes without requiring negative-case endpoints to exist."""
    ...
    for case in repository["semantic_cases"]:
        source = case["source"]
        target = case["target"]
        endpoints_present = source in node_paths and target in node_paths
        ...
        results.append(
            {
                ...
                "source_endpoint_present": source in node_paths,
                "target_endpoint_present": target in node_paths,
...
def aggregate_semantics(repositories: Sequence[Mapping[str, Any]]) -> Mapping[str, Any]:
    ...
    return {"aggregate": aggregate, "languages": {"typescript": dict(aggregate)}}
...
def evaluate_policy(
    ...
    for scope, result in (("aggregate", semantics["aggregate"]), ("typescript", semantics["languages"]["typescript"])):
        maximum(f"semantic.{scope}.false_positives", result["fp"], semantic_policy["false_positives_max"])
        maximum(f"semantic.{scope}.false_negatives", result["fn"], semantic_policy["false_negatives_max"])
        for metric in ("micro_precision", "micro_recall", "macro_precision", "macro_recall"):
            minimum(f"semantic.{scope}.{metric}", result[metric], semantic_policy[f"{metric}_min"])
    ...
    for repository in repositories:
        ...
        maximum(f"performance.{repository_id}.wall_max_ms", summary["wall_max_ms"], defaults["analyze_max_ms"])
        maximum(f"performance.{repository_id}.wall_median_ms", summary["wall_median_ms"], defaults["analyze_median_max_ms"])
        maximum(f"performance.{repository_id}.peak_rss_max_kib", summary["peak_rss_max_kib"], defaults["rss_max_kib"])
        maximum(f"determinism.{repository_id}.artifact_unique_digests", summary["artifact_unique_digests"], determinism["artifact_unique_digests_max"])
        maximum(f"determinism.{repository_id}.semantic_unique_digests", summary["semantic_unique_digests"], determinism["semantic_unique_digests_max"])
        maximum(f"determinism.{repository_id}.unique_node_edge_count_pairs", summary["unique_node_edge_count_pairs"], determinism["unique_node_edge_count_pairs_max"])
        for run in repository["runs"]:
            maximum(f"performance.{repository_id}.run_{run['ordinal']}.artifact_bytes", run["graph"]["artifact_bytes"], policy["performance"]["graph_artifact_max_bytes"])
            maximum(f"performance.{repository_id}.run_{run['ordinal']}.inspect_ms", run["inspect_wall_ms"], policy["performance"]["inspect_max_ms"])
    maximum(
...
def benchmark(args: argparse.Namespace) -> Mapping[str, Any]:
    ...
    repositories = []
    with tempfile.TemporaryDirectory(prefix="bitcode-typescript-") as temporary_name:
    ...
    return {
        "schema_version": 1,
        "kind": "typescript-support-observation",
